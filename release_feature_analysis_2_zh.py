...
def generate_summary_plot(shap_values, X, filename='release_figures/cut6_or_mo_sup_1_SHAP_summary.png'):
    ...
    ax.set_xlabel("SHAP值", fontsize=sz)
    ax.set_title('特征重要性 - 药物释放百分比', pad=20)

    # 定制颜色条
    cb_ax = fig.axes[1]
    cb_ax.tick_params(labelsize=sz)
    cb_ax.set_yticklabels(['低', '高'], fontsize=sz)
    cb_ax.set_ylabel("特征值", fontsize=sz)
...
# 调用SHAP绘图函数
    ...
    ax = plt.gca()
    ax.tick_params(labelsize=sz-3)
    ax.set_xlabel("Fractional Release", fontsize=sz, color='black')
    ax.set_title("Feature Decision Tree for Sample 2",fontsize=sz+3)
    # 保存并关闭图表
    plt.tight_layout()
    plt.savefig(filename, dpi=600, format='png', transparent=False)
    plt.close()
...
if __name__ == '__main__':
    ...
    generate_summary_plot(shap_values, X,
                          filename='release_feature_figures_zh/zh_sup_6m_SHAP_summary.png')
