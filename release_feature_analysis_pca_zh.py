...
if __name__ == '__main__':
    ...
    OUTPUT_FIGURE_FILENAME = "zh_cumulative_explained_variance_plot.png"
    # ================================================================
    ...
    plt.plot(component_indices, cumulative_explained_variance,
             label='累计解释方差', color='red', marker='o', linestyle='-')
...
# 绘制90%和95%的参考线
    plt.axhline(y=0.9999, color='purple', linestyle='--', label=f'99.99%:{n_components_9999}主成分')
    plt.axhline(y=0.95,  color='g', linestyle='--', label=f'95%:{n_components_95}主成分')
    plt.axhline(y=0.90,  color='orange', linestyle='--', label=f'90%:{n_components_90}主成分')


    plt.ylabel('解释方差比例')
    plt.xlabel('主成分数量')
    plt.title('累计解释方差图 (主成分分析)')
