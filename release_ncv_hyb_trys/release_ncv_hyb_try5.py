...
class HybridAutoregressiveCV:
    ...
    def _prepare_gru_teacher_forcing_data(self, sample_ids, feature_scaler, release_scaler):
        X_list, y_list, groups_list = [], [], [] # <--- 修改: 增加groups_list
        for sid in sample_ids:
            group = self.df[self.df['sample_id'] == sid]
            drug_name = group['drug_name'].iloc[0] # <--- 修改: 获取drug_name用于分组
            ...
            if len(releases_s) > self.SEQ_LENGTH:
                for j in range(len(releases_s) - self.SEQ_LENGTH):
                    ...
                    groups_list.append(drug_name) # <--- 修改: 为每个序列样本添加其组标签
    ...
    def cross_validation(self, num_trials=10, num_prefix_points=3):
        ...
        for i, (train_ids_idx, test_ids_idx) in enumerate(outer_cv.split(all_sample_ids, groups=drug_groups)):
            ...
            # <--- 修改: 现在 _prepare_gru_teacher_forcing_data 返回分组信息
            X_gru_train, y_gru_train, G_gru_train = self._prepare_gru_teacher_forcing_data(train_sample_ids, feature_scaler, release_scaler)
            input_size = X_gru_train.shape[2]

            # <--- 修改: 用RandomizedSearchCV替换掉固定的GRU训练
            print("  > 正在为GRU特征提取器搜索最佳参数...")
            base_gru_net = NeuralNetRegressor(
                GRUFeatureExtractor,
            ...
            gru_search = RSCV(
                base_gru_net,
                self.gru_p_grid,
                n_iter=30, # GRU的搜索迭代次数，可按需调整
                scoring='neg_mean_absolute_error',
                cv=GroupKFold(n_splits=5), # 对GRU的内层CV使用分组
                n_jobs=-1,
                refit=True,
                random_state=i
            )
            ...
            gru_feature_extractor = gru_search.best_estimator_ # 获取最优GRU模型
            ...
            lgbm_search = RSCV(self.lgbm, self.lgbm_p_grid, n_iter=30, scoring='neg_mean_absolute_error',
            ...
            for sid in test_sample_ids:
                ...
                predicted_curve = self._predict_autoregressive_hybrid(best_lgbm_model, gru_feature_extractor, sid, feature_scaler, release_scaler, num_prefix_points)
            ...
            for j in range(len(nested_E)):
                ...
                flat_G.extend([nested_G[j]] * num_points)
                flat_E.extend([nested_E[j]] * num_points)
                flat_T.extend(nested_T[j])
                flat_y.extend(nested_y[j])
                flat_pred.extend(nested_pred[j])
            ...
            self.G_test_list.append(np.array(flat_G))
            self.E_test_list.append(np.array(flat_E))
            self.T_test_list.append(np.array(flat_T))
            self.y_test_list.append(np.array(flat_y))
            self.pred_list.append(np.array(flat_pred))

    # [ 其他方法 _predict_autoregressive_hybrid, results, best_model 保持不变 ]
    ...
    def best_model(self):
        print("\n--- 正在训练最终生产模型 ---")
        # 注意：此处的最终模型训练仍使用固定的GRU参数。
        # 一个更完整的实现会利用CV中找到的最佳参数。为保持改动最小，此处暂不修改。
        all_sample_ids = self.df['sample_id'].unique()
        feature_scaler = StandardScaler(); release_scaler = IdentityScaler()
        feature_scaler.fit(self.df[self.static_feature_columns])
        release_scaler.fit(self.df[['release_percentage']])
        X_gru_all, y_gru_all, _ = self._prepare_gru_teacher_forcing_data(all_sample_ids, feature_scaler, release_scaler)
        input_size = X_gru_all.shape[2]
        final_gru_extractor = NeuralNetRegressor(
            GRUFeatureExtractor, module__input_size=input_size, criterion=torch.nn.L1Loss,
            optimizer=torch.optim.Adam, max_epochs=100, lr=0.001, device=self.device, verbose=0,
            module__hidden_size=64, module__num_layers=2 # 使用固定的默认值
        )
