...
class HybridAutoregressiveCV:
    ...
    def cross_validation(self, num_trials=10, num_prefix_points=3):
        ...
        for i, (train_ids_idx, test_ids_idx) in enumerate(outer_cv.split(all_sample_ids, groups=drug_groups)):
            ...
            gru_search = RSCV(base_gru_net, self.gru_p_grid, n_iter=10, scoring='neg_mean_absolute_error', cv=GroupKFold(n_splits=5), n_jobs=-1, refit=True, random_state=i)
    ...
    def results(self):
        ...
        self.CV_dataset = pd.DataFrame(list_of_tuples, columns=['Iter', 'Valid Score', 'Test Score', 'Model Parms', 'DP_Groups', "Experimental Index", "Time", 'Experimental_Release', 'Predicted_Release'])
    ...
    def best_model(self):
        ...
        # <--- 修改: 将 time_delta_scaler 传入
        X_gru_all, y_gru_all, G_gru_all = self._prepare_gru_teacher_forcing_data(all_sample_ids, feature_scaler, release_scaler, time_delta_scaler)
        input_size = X_gru_all.shape[2]

        # 寻找全局最佳GRU参数并训练
        print("  > 正在为最终GRU模型搜索最佳参数...")
        base_gru_net = NeuralNetRegressor(GRUFeatureExtractor, module__input_size=input_size, criterion=torch.nn.L1Loss, optimizer=torch.optim.Adam, device=self.device, verbose=0)
        final_gru_search = RSCV(base_gru_net, self.gru_p_grid, n_iter=10, scoring='neg_mean_absolute_error', cv=GroupKFold(n_splits=10), n_jobs=-1, refit=True, random_state=42)
        final_gru_search.fit(X_gru_all, y_gru_all, groups=G_gru_all)
        final_gru_extractor = final_gru_search.best_estimator_
        print(f"  > 已找到并训练最终GRU模型，最佳参数: {final_gru_search.best_params_}")
        ...
        self.final_model = {
            ...
            # <--- 修改: 将 time_delta_scaler 添加到最终模型中
...
if __name__ == '__main__':
    config = {
        ...
        "gru_hyperparameters": {
            ...
            'batch_size': [16, 32], 'module__hidden_size': [32, 64, 128],
