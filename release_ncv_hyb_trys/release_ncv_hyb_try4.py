...
import joblib
# 固定了参数，cv绘图没问题，预测real效果不好
# release去除标准化后训练不稳定，考虑针对feature的标准化进行调整，比如只针对静态特征标准化，release传过来的不标准化
...
class HybridAutoregressiveCV:
    # *** 构造函数现在接收超参数网格 ***
    def __init__(self, datafile, static_feature_columns, seq_len, gru_train_params, lgbm_p_grid):
        ...
        # 保存传入的参数
        self.gru_train_params = gru_train_params
...
def run_hybrid_cv(config):
    model_instance = HybridAutoregressiveCV(
        ...
        # *** 将超参数空间传入 ***
        gru_train_params=config['gru_train_parameters'],
...
if __name__ == '__main__':
    ...
    config = {
        ...
        # 3. GRU 特征提取器训练参数 (现在可自定义)
        # 注意：这里不是搜索空间，而是固定的训练参数
        "gru_train_parameters": {
            'max_epochs': 100,
            'lr': 0.001,
            'batch_size': 32,
