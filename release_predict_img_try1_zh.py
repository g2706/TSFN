...
if __name__ == '__main__':
    ...
    for name, group in grouped:
        ...
        ax.plot(X1, Y1, label='预测值', linestyle='--', marker='o', markersize=8,
                markeredgecolor="black", alpha=0.8)

        ax.plot(X2, Y2, label='实验值', linestyle='--', marker='o', markersize=8,
                alpha=0.8, markeredgecolor="black")

        # y-axis limits and interval
        ax.set(ylim=(-0.02, 1.08), yticks=np.arange(0, 1.08, 0.1))
        ax.set_xlabel('时间 (h)', fontsize=15, color='black')
        ax.set_ylabel('药物累计释放百分比', fontsize=15, color='black')
        ax.spines['top'].set_visible(True)
        ax.spines['right'].set_visible(True)
        ax.set_title(str(dp) + ' (实验样本' + str(name) + '号)', color='black',
                     fontsize=18, pad=20)
        ...
        plt.tight_layout()
        os.makedirs("release_Figures_real_predict_zh", exist_ok=True)
        plt.savefig('release_Figures_real_predict_zh/zh_cut5_sup_real_hyb_release_EXP-INDEX' + str(name) + '.png', dpi=600,
                    transparent=False)
