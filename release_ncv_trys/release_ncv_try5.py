...
def run_NESTED_CV(name, CV):
    ...
    # 保存结果
    result_filename = f"release_NESTED_CV_RESULTS/GRU_sequence_model_results.pkl"
    model_instance.CV_dataset.to_pickle(result_filename)
    print(f"\nCV 结果已保存至: {result_filename}")

    # 保存模型
    model_filename = f"release_Trained_models/GRU_sequence_model_final.pkl"
...
if __name__ == '__main__':
    ...
    result, model = run_NESTED_CV("GRU", CV=10)
