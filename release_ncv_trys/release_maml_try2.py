...
# --- 步骤3: 数据准备 (现在接收 seq_len 和 k_shot) ---
    ...
    def __init__(self, df, static_feature_columns, seq_len, k_shot):
        ...
        for sample_id, group in df.groupby('sample_id'):
            ...
            for j in range(num_sequences):
                seq_in_release = releases_s[j : j + self.seq_len]
                seq_out_release = releases_s[j + self.seq_len]
                combined_seq = []
...
if __name__ == '__main__':
    ...
    # 2. 定义用于适应新任务的前N个真实数据点
    NUM_ADAPTATION_POINTS = 6 # 例如，使用前4个点 (p1, p2, p3, p4)

    # 3. 其他超参数
    NUM_ADAPTATION_STEPS = 20 # 在新任务上微调的步数
    META_EPOCHS = 200
    # ================================================================

    # --- 自动计算并验证参数 ---
    if NUM_ADAPTATION_POINTS <= SEQ_LEN:
        raise ValueError(f"NUM_ADAPTATION_POINTS ({NUM_ADAPTATION_POINTS}) 必须大于 SEQ_LEN ({SEQ_LEN})")
    # k_shot: 从N个点中，可以构建出 N - S 个训练样本
    K_SHOT = NUM_ADAPTATION_POINTS - SEQ_LEN
    print(f"配置: GRU输入长度(SEQ_LEN)={SEQ_LEN}, 适应点数(NUM_ADAPTATION_POINTS)={NUM_ADAPTATION_POINTS}")
    print(f"  => 自动计算得出，每个任务将使用 {K_SHOT} 个样本进行快速适应 (k_shot)。")
    ...
    full_dataset = CurveTaskDataset(df, static_cols, seq_len=SEQ_LEN, k_shot=K_SHOT)
    print(f"数据加载完成，过滤后剩余 {len(full_dataset)} 个有效任务。")

    tasks = list(range(len(full_dataset))); random.shuffle(tasks)
    ...
    trainer = MAMLTrainer(model=meta_model, meta_lr=1e-3, fast_lr=0.001, num_updates=5)
    ...
    adapted_model.eval()
    with torch.no_grad():
        full_times = test_task['full_times']
        full_releases_s = test_task['full_releases_scaled']
        static_features_s = test_task['static_features_scaled']
        ...
        for i in range(NUM_ADAPTATION_POINTS - 1, len(full_times) - 1):
            ...
            # 滚动更新序列
            current_release_seq_s.pop(0)
            current_release_seq_s.append(next_pred_s)
    ...
    plt.plot(full_times, pred_curve_raw, 'ro--', label='Meta-Learned Autoregressive Prediction')
