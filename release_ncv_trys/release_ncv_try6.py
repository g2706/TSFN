...
class NESTED_CV:
    ...
    def input_target(self):
        """
        重写此方法以创建序列数据，并使用 drug_name 作为分组依据。
        """
        print("正在创建序列数据...")
        ...
        # *** 核心修改在这里 ***
        # 我们仍然按 sample_id 遍历来确保样本不被拆散
        # 但我们使用 drug_name 作为分组的标签
    ...
    # cross_validation, results, best_model 等方法无需任何修改
    ...
    def cross_validation(self, input_value):
        ...
        for i in range(NUM_TRIALS):
            ...
            for train_index, test_index in cv_outer.split(self.X, self.Y, self.G):
                ...
                search = RSCV(self.user_defined_model, self.p_grid, n_iter=10, verbose=0,
...
def run_NESTED_CV(name, CV):
    ...
    result_filename = f"release_NESTED_CV_RESULTS/re_GRU_sequence_model_results_by_drug_name.pkl"
    model_instance.CV_dataset.to_pickle(result_filename)
    print(f"\nCV 结果已保存至: {result_filename}")

    model_filename = f"release_Trained_models/re_GRU_sequence_model_final_by_drug_name.pkl"
