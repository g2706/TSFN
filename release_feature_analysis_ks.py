...
def plot_ks_curve(data1, data2, name1='Data 1', name2='Data 2', output_path='ks_plot.png'):
    ...
    plt.step(data1_sorted, cdf1, label=f'{name1} CDF', where='post')
    plt.step(data2_sorted, cdf2, label=f'{name2} CDF', where='post')
    ...
    plt.vlines(x_max_diff, min(y1_max_diff, y2_max_diff), max(y1_max_diff, y2_max_diff),
               color='red', linestyle='--', label=f'Max Difference = {ks_statistic:.4f}')

    plt.title(f'K-S Inspection: {name1} vs {name2}')
    plt.xlabel('Fractional Release')
    plt.ylabel('Cumulative Probability')
    plt.text(0.5, 0.95, f'P-Value = {p_value:.4f}', fontsize=sz-2, horizontalalignment='center', verticalalignment='top')
    plt.legend(loc='best')
...
if __name__ == '__main__':
    ...
    plot_ks_curve(
        predicted_values,
        experimental_values,
        name1='Predicted Value',
        name2='Experimental Value',
        output_path=os.path.join(OUTPUT_FOLDER, 'ks_plot_prediction_vs_experiment.png')
    )